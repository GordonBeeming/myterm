...
final class WorkspaceStoreStressTests: XCTestCase {
    ...
    func testAgentTitleOfTenThousandCharactersIsCappedByScalarNotByte() {
        ...
        // Grapheme clusters each built from one base and 50 combining marks: the cap counts
        // scalars, so one cluster of 51 scalars cannot smuggle a payload 51 times the cap.
        ...
        XCTAssertLessThanOrEqual(sanitized?.count ?? 0, AgentSessionTitle.maximumLength / 51 + 1)
    }
