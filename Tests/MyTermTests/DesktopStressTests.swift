...
final class DesktopStressTests: XCTestCase {
    ...
    func testRenameCommitsNothingWhenADeviceDeletedTheWorkspaceMidRename() throws {
        ...
        XCTAssertTrue(model.deleteWorkspace(workspaceID: victim.description))
    ...
    func testADragEndsQuietlyAfterADeviceDeletedTheWorkspace() throws {
        ...
        XCTAssertTrue(model.deleteWorkspace(workspaceID: workspaceID.description))
        XCTAssertNil(model.paneTabDragSession)
    ...
    func testNotificationPopoverHeightForEveryCountTheBellCanShow() {
        let row: CGFloat = 44
        let divider: CGFloat = 1
        func height(_ count: Int) -> CGFloat {
            AgentNotificationsScrollLayout.scrollHeight(
                rowHeights: Array(repeating: row, count: min(count, AgentNotificationsScrollLayout.maxVisibleRows)),
                dividerHeight: divider,
                totalRowCount: count
            )
        }
        XCTAssertEqual(height(0), 0)
        XCTAssertEqual(height(1), row)
        XCTAssertEqual(height(5), 5 * row + 4 * divider)
        XCTAssertEqual(height(6), 5 * row + 5 * divider + row * 0.4)
        XCTAssertEqual(height(200), height(6), "Two hundred rows are no taller than six")
        XCTAssertEqual(height(AgentNotificationInbox.capacity), height(6))
    }

    func testTheInboxNeverHoldsMoreThanItsCapacityAndTheBellCountCanPassNinetyNine() throws {
        let harness = try AgentTestHarness { directory in
        ...
        let workspaceID = model.store.selectedWorkspaceID
        let groupID = model.selectedWorkspace.focusedTabGroupID
        // One entry per tab, so nothing supersedes: 250 tabs, 250 finished turns.
        for _ in 0..<249 { model.createTerminalTab(in: groupID) }
        let tabs = try XCTUnwrap(model.selectedWorkspace.group(id: groupID)?.tabs)
        XCTAssertEqual(tabs.count, 250)
        for tab in tabs {
            harness.record(.finished, workspaceID: workspaceID, tabGroupID: groupID, tabID: tab.id)
        }
        XCTAssertEqual(model.agentNotificationCount, AgentNotificationInbox.capacity)
        XCTAssertGreaterThan(model.agentNotificationCount, 99, "The bell has no 99+ cap; it shows the full count")

        // Clear All while an entry arrives: the arrival lands after the clear and stays unread.
        model.clearAgentNotifications()
        XCTAssertEqual(model.agentNotificationCount, 0)
        harness.record(.awaitingInput, workspaceID: workspaceID, tabGroupID: groupID, tabID: tabs[0].id)
        XCTAssertEqual(model.agentNotificationCount, 1)
    }

    func testTogglingThePopoverFiftyTimesLeavesItInAKnownState() throws {
        let directory = try makeTemporaryDirectory()
        defer { try? FileManager.default.removeItem(at: directory) }
        let model = try makeModel(applicationSupportDirectory: directory)
        // The menu item is "Show", not "Toggle": pressing ⇧⌘I fifty times leaves it shown.
        for _ in 0..<50 { model.isAgentNotificationsPresented = true }
        XCTAssertTrue(model.isAgentNotificationsPresented)
        XCTAssertTrue(model.agentNotificationItems.isEmpty, "Opens onto the empty state when nothing waits")
        // The bell toggles, so fifty bell clicks come back to closed.
        for _ in 0..<50 { model.isAgentNotificationsPresented.toggle() }
        XCTAssertTrue(model.isAgentNotificationsPresented)
        model.isAgentNotificationsPresented.toggle()
        XCTAssertFalse(model.isAgentNotificationsPresented)
    }

    func testTurningNotificationsOffWhileOneIsPendingStopsTheNextBannerOnly() throws {
        let harness = try AgentTestHarness { directory in
            addTeardownBlock { try? FileManager.default.removeItem(at: directory) }
        }
        let model = harness.model
        ...
        harness.record(.awaitingInput, workspaceID: workspaceID, tabGroupID: groupID, tabID: tabs[1].id)
        XCTAssertEqual(harness.poster.posted.count, 1)
        XCTAssertEqual(model.agentNotificationCount, 1)

        harness.notifications.isEnabled = false
        harness.record(.finished, workspaceID: workspaceID, tabGroupID: groupID, tabID: tabs[0].id)
        XCTAssertEqual(harness.poster.posted.count, 1, "No banner once the setting is off")
        XCTAssertEqual(model.agentNotificationCount, 2, "The bell still files the entry")

        harness.notifications.isEnabled = true
        harness.record(.finished, workspaceID: workspaceID, tabGroupID: groupID, tabID: tabs[1].id)
        XCTAssertEqual(harness.poster.posted.count, 2, "Back on, the next report posts again")
        XCTAssertEqual(model.agentActivity(forTab: tabs[1].id), .awaitingInput, "The bell keeps the question over the finished turn")
        XCTAssertEqual(model.agentNotificationCount, 2)
    }

    // MARK: - Agent titles at speed

    /// Every distinct title is a store write, and the store writes synchronously on the main
    /// thread. The dedupe on identical titles is the only throttle there is.
    func testAHundredTitleChangesASecondWriteOnlyWhenTheTitleChanges() throws {
        let harness = try AgentTestHarness { directory in
            addTeardownBlock { try? FileManager.default.removeItem(at: directory) }
        }
        let model = harness.model
        let workspaceID = model.store.selectedWorkspaceID
        let groupID = model.selectedWorkspace.focusedTabGroupID
        let tab = try XCTUnwrap(model.selectedWorkspace.group(id: groupID)?.tabs.first)
        let sessionID = try XCTUnwrap(tab.terminalSession?.id)
        model.recordAgentPresence(
            AgentActivityReport(agent: "claude", activity: .working),
            workspaceID: workspaceID,
            tabGroupID: groupID,
            tabID: tab.id
        )

        let before = model.stateVersion
        for _ in 0..<100 {
            model.recordAgentTitle("✳ Same topic", workspaceID: workspaceID, tabGroupID: groupID, tabID: tab.id, sessionID: sessionID)
        }
        XCTAssertEqual(model.stateVersion, before + 1, "Identical titles cost one write")
        XCTAssertEqual(model.selectedWorkspace.selectedTab?.automaticDisplayTitle, "Same topic")

        let distinctBefore = model.stateVersion
        let clock = ContinuousClock()
        let elapsed = clock.measure {
            for index in 0..<100 {
                model.recordAgentTitle("✳ Topic \(index)", workspaceID: workspaceID, tabGroupID: groupID, tabID: tab.id, sessionID: sessionID)
            }
        }
        XCTAssertEqual(model.stateVersion, distinctBefore + 100, "Distinct titles are one write each, no coalescing")
        // A hundred writes on a one-workspace store; the 500-workspace cost is measured in
        // WorkspaceStoreStressTests.testAgentTitleWriteOnALargeStoreStaysCheap.
        XCTAssertLessThan(elapsed, .seconds(1), "100 distinct titles took \(elapsed)")
    }

    // MARK: - Settings: Devices

    func testRelayAddressExtremes() {
        XCTAssertNil(AppModel.relayURL(from: ""))
        XCTAssertNil(AppModel.relayURL(from: "   \n\t "))
        XCTAssertNil(AppModel.relayURL(from: String(repeating: "a", count: 10_000)))
        XCTAssertNil(AppModel.relayURL(from: "ftp://relay.example"))
        XCTAssertNil(AppModel.relayURL(from: "https://"))
        XCTAssertEqual(AppModel.relayURL(from: "  wss://relay.example/path?x=1#f  ")?.absoluteString, "https://relay.example")
        XCTAssertEqual(AppModel.relayURL(from: "HTTP://relay.example:8787/")?.absoluteString, "http://relay.example:8787")
        let longHost = "https://" + String(repeating: "a", count: 10_000) + ".example"
        if let url = AppModel.relayURL(from: longHost) {
            XCTAssertEqual(url.host?.count, 10_008)
        }
    }

    func testSettingARelayThatDoesNotParseTurnsTheRelayOffAndKeepsTheText() throws {
        let directory = try makeTemporaryDirectory()
        defer { try? FileManager.default.removeItem(at: directory) }
        let model = try makeModel(applicationSupportDirectory: directory)
        model.setRelay(urlText: "https://relay.example", enabled: true)
        XCTAssertTrue(model.isRelayEnabled)
        model.setRelay(urlText: "   ", enabled: true)
        XCTAssertFalse(model.isRelayEnabled)
        XCTAssertEqual(model.relayURLText, "")
        let long = String(repeating: "x", count: 10_000)
        model.setRelay(urlText: long, enabled: true)
        XCTAssertFalse(model.isRelayEnabled)
        XCTAssertEqual(model.relayURLText, long, "The text is kept verbatim for the user to fix")
