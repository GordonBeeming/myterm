...
final class AgentInboxPersistenceTests: XCTestCase {
    ...
    func testAnOversizedHistoryIsCappedOnLoad() throws {
        // The cap is the inbox's own promise about its size, and a file is not trusted to have
        // kept it.
    ...
    func testTheHistoryComesBackReadAfterARelaunch() throws {
        ...
        XCTAssertEqual(relaunched.agentInbox.history.map(\.isRead), [true])
    }
}
