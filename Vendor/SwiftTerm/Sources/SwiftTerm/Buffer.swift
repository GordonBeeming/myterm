...
extension Buffer {
    func makeCheckpoint() throws -> CheckpointBuffer {
        var checkpointLines: [CheckpointLine] = []
        checkpointLines.reserveCapacity(_lines.count)
        for index in 0..<_lines.count {
            checkpointLines.append(try _lines[index].makeCheckpoint())
        }
        return CheckpointBuffer(
            columns: _cols,
            rows: _rows,
            scrollback: scrollback,
            maximumLines: _lines.maxLength,
            lines: checkpointLines,
            xDisplay: xDisp,
            yDisplay: _yDisp,
            xBase: xBase,
            x: _x,
            ...
            savedX: savedX,
            savedY: savedY,
            savedOriginMode: savedOriginMode,
            savedMarginMode: savedMarginMode,
            savedWraparound: savedWraparound,
            savedReverseWraparound: savedReverseWraparound,
            marginLeft: _marginLeft,
            marginRight: _marginRight,
            savedAttribute: CheckpointAttribute(savedAttr),
            savedCharset: savedCharset.map(CheckpointCharset.init),
            currentAttribute: CheckpointAttribute(curAttr),
            insertMode: insertMode,
            marginMode: marginMode,
            wraparound: wraparound,
            lastStorageY: lastBufferStorage.y,
            lastStorageX: lastBufferStorage.x,
            lastStorageColumns: lastBufferStorage.cols,
            lastStorageRows: lastBufferStorage.rows
        )
    }

    static func restore(from snapshot: CheckpointBuffer, tabStopWidth: Int,
                        hyperlinkAtoms: inout [String: TinyAtom]) throws -> Buffer {
        let buffer = Buffer(
        ...
        buffer._lines = CircularBufferLineList(maxLength: snapshot.maximumLines)
        buffer._lines.makeEmpty = { [unowned buffer] _ in
            buffer.getBlankLine(attribute: CharData.defaultAttr, isWrapped: false)
        }
        buffer.setupLinesCallbacks()
        for lineSnapshot in snapshot.lines {
            let line = try BufferLine.restore(from: lineSnapshot, hyperlinkAtoms: &hyperlinkAtoms)
            guard line.count == snapshot.columns else {
                throw TerminalCheckpointError.corrupt("line width does not match its buffer")
            }
            buffer._lines.push(line)
        }
        buffer.xDisp = snapshot.xDisplay
        buffer._yDisp = snapshot.yDisplay
        buffer.xBase = snapshot.xBase
        buffer._x = snapshot.x
        buffer._y = snapshot.y
        buffer._yBase = snapshot.yBase
        buffer.linesTop = snapshot.linesTop
        buffer._scrollBottom = snapshot.scrollBottom
        buffer._scrollTop = snapshot.scrollTop
        buffer.tabStops = snapshot.tabStops
        buffer.savedX = snapshot.savedX
        buffer.savedY = snapshot.savedY
        buffer.savedOriginMode = snapshot.savedOriginMode
        buffer.savedMarginMode = snapshot.savedMarginMode
        buffer.savedWraparound = snapshot.savedWraparound
        buffer.savedReverseWraparound = snapshot.savedReverseWraparound
        buffer._marginLeft = snapshot.marginLeft
        buffer._marginRight = snapshot.marginRight
        buffer.savedAttr = try snapshot.savedAttribute.value()
        buffer.savedCharset = snapshot.savedCharset?.value()
        buffer.curAttr = try snapshot.currentAttribute.value()
        buffer.insertMode = snapshot.insertMode
        buffer.marginMode = snapshot.marginMode
        buffer.wraparound = snapshot.wraparound
        buffer.lastBufferStorage = (
            snapshot.lastStorageY,
            snapshot.lastStorageX,
            snapshot.lastStorageColumns,
            snapshot.lastStorageRows
        )
