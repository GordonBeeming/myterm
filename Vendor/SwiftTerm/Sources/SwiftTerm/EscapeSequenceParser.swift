...
extension EscapeSequenceParser {
    func makeCheckpoint(pendingUtf8: [UInt8]) throws -> CheckpointParser {
        let handlerName: String?
        let handlerData: [UInt8]
        switch activeDcsHandler {
        case nil:
            handlerName = nil
            handlerData = []
        case let handler as Terminal.DECRQSS:
            handlerName = "decrqss"
            handlerData = handler.data
        case let handler as SixelDcsHandler:
            handlerName = "sixel"
            handlerData = handler.data
        default:
            throw TerminalCheckpointError.unsupportedParserHandler
        }
        return CheckpointParser(
            ...
            apc: _apc,
            parameters: _pars,
            parameterText: _parsTxt,
            collect: _collect,
            pendingUtf8: pendingUtf8,
            dcsHandler: handlerName,
            dcsData: handlerData
        )
    }

    func prepareRestoration(from snapshot: CheckpointParser, terminal: Terminal) throws -> () -> Void {
        guard let initial = ParserState(rawValue: snapshot.initialState),
              let current = ParserState(rawValue: snapshot.currentState) else {
            throw TerminalCheckpointError.corrupt("unknown parser state")
        }
        let handler: DcsHandler?
        switch snapshot.dcsHandler {
        case nil:
            handler = nil
        case "decrqss":
            let restored = Terminal.DECRQSS(terminal: terminal)
            restored.data = snapshot.dcsData
            handler = restored
        case "sixel":
            let restored = SixelDcsHandler(terminal: terminal)
            restored.data = snapshot.dcsData
            handler = restored
        default:
            throw TerminalCheckpointError.corrupt("unknown DCS handler")
        }
        if current == .dcsPassthrough && handler == nil {
            throw TerminalCheckpointError.corrupt("DCS passthrough has no handler")
        }
        return { [self] in
            ...
            _osc = snapshot.osc
            _apc = snapshot.apc
            _pars = snapshot.parameters
            _parsTxt = snapshot.parameterText
            _collect = snapshot.collect
            activeDcsHandler = handler
        }
    }
}
