...
extension KittyGraphicsControl {
    var checkpoint: CheckpointKittyControl {
        CheckpointKittyControl(
            action: String(action), suppressResponses: suppressResponses, format: format,
            transmission: String(transmission), width: width, height: height,
            cropX: cropX, cropY: cropY, cropWidth: cropWidth, cropHeight: cropHeight,
            dataSize: dataSize, dataOffset: dataOffset, imageId: imageId,
            imageNumber: imageNumber, placementId: placementId, parentImageId: parentImageId,
            parentPlacementId: parentPlacementId, offsetH: offsetH, offsetV: offsetV,
            pixelOffsetX: pixelOffsetX, pixelOffsetY: pixelOffsetY,
            unicodePlaceholder: unicodePlaceholder, zIndex: zIndex, more: more,
            compression: compression.map(String.init), columns: columns, rows: rows,
            cursorPolicy: cursorPolicy, deleteMode: deleteMode.map(String.init)
        )
    }

    static func restore(from value: CheckpointKittyControl) throws -> KittyGraphicsControl {
        guard value.action.count == 1, let action = value.action.first,
              value.transmission.count == 1, let transmission = value.transmission.first,
              value.compression == nil || value.compression?.count == 1,
              value.deleteMode == nil || value.deleteMode?.count == 1 else {
            throw TerminalCheckpointError.corrupt("invalid Kitty control character")
        }
        return KittyGraphicsControl(
            action: action, suppressResponses: value.suppressResponses, format: value.format,
            transmission: transmission, width: value.width, height: value.height,
            cropX: value.cropX, cropY: value.cropY, cropWidth: value.cropWidth,
            cropHeight: value.cropHeight, dataSize: value.dataSize, dataOffset: value.dataOffset,
            imageId: value.imageId, imageNumber: value.imageNumber, placementId: value.placementId,
            parentImageId: value.parentImageId, parentPlacementId: value.parentPlacementId,
            offsetH: value.offsetH, offsetV: value.offsetV, pixelOffsetX: value.pixelOffsetX,
            pixelOffsetY: value.pixelOffsetY, unicodePlaceholder: value.unicodePlaceholder,
            zIndex: value.zIndex, more: value.more, compression: value.compression?.first,
            columns: value.columns, rows: value.rows, cursorPolicy: value.cursorPolicy,
            deleteMode: value.deleteMode?.first
        )
    }
}

extension KittyGraphicsState {
    func makeCheckpoint() -> CheckpointKittyState {
        let images = imagesById.map { id, image -> CheckpointKittyImage in
            switch image.payload {
            case .png(let data):
                return CheckpointKittyImage(id: id, kind: 0, data: data, width: 0, height: 0,
                                            byteSize: image.byteSize, lastAccessTick: image.lastAccessTick)
            case .rgba(let bytes, let width, let height):
                return CheckpointKittyImage(id: id, kind: 1, data: Data(bytes), width: width, height: height,
                                            byteSize: image.byteSize, lastAccessTick: image.lastAccessTick)
            }
        }.sorted { $0.id < $1.id }
        let numbers = imageNumbers.map { CheckpointKittyNumber(number: $0.key, imageId: $0.value) }
            .sorted { $0.number < $1.number }
        let placements = placementsByKey.values.map {
            CheckpointKittyPlacement(
                imageId: $0.imageId, placementId: $0.placementId,
                parentImageId: $0.parentImageId, parentPlacementId: $0.parentPlacementId,
                parentOffsetH: $0.parentOffsetH, parentOffsetV: $0.parentOffsetV,
                pixelOffsetX: $0.pixelOffsetX, pixelOffsetY: $0.pixelOffsetY,
                column: $0.col, row: $0.row, columns: $0.cols, rows: $0.rows,
                zIndex: $0.zIndex, isVirtual: $0.isVirtual,
                isAlternateBuffer: $0.isAlternateBuffer
            )
        }.sorted { ($0.imageId, $0.placementId) < ($1.imageId, $1.placementId) }
        let pending = pending.map {
            CheckpointKittyPending(control: $0.control.checkpoint, base64Payload: $0.base64Payload)
        }
        return CheckpointKittyState(
            images: images, imageNumbers: numbers, nextImageId: nextImageId,
            nextPlacementId: nextPlacementId, pending: pending, placements: placements,
            totalImageBytes: totalImageBytes, nextImageAccessTick: nextImageAccessTick
        )
    }

    static func restore(from snapshot: CheckpointKittyState) throws -> KittyGraphicsState {
        let state = KittyGraphicsState()
        for image in snapshot.images {
            let payload: KittyGraphicsPayload
            switch image.kind {
            case 0: payload = .png(image.data)
            case 1: payload = .rgba(bytes: [UInt8](image.data), width: image.width, height: image.height)
            default: throw TerminalCheckpointError.corrupt("unknown Kitty image kind")
            }
            state.imagesById[image.id] = KittyGraphicsImage(
                payload: payload, byteSize: image.byteSize, lastAccessTick: image.lastAccessTick
            )
        }
        for number in snapshot.imageNumbers {
            state.imageNumbers[number.number] = number.imageId
        }
        for placement in snapshot.placements {
            let key = KittyPlacementKey(imageId: placement.imageId, placementId: placement.placementId)
            state.placementsByKey[key] = KittyPlacementRecord(
                imageId: placement.imageId, placementId: placement.placementId,
                parentImageId: placement.parentImageId, parentPlacementId: placement.parentPlacementId,
                parentOffsetH: placement.parentOffsetH, parentOffsetV: placement.parentOffsetV,
                pixelOffsetX: placement.pixelOffsetX, pixelOffsetY: placement.pixelOffsetY,
                col: placement.column, row: placement.row, cols: placement.columns, rows: placement.rows,
                zIndex: placement.zIndex, isVirtual: placement.isVirtual,
                isAlternateBuffer: placement.isAlternateBuffer
            )
        }
        state.nextImageId = snapshot.nextImageId
        state.nextPlacementId = snapshot.nextPlacementId
        if let pending = snapshot.pending {
            state.pending = KittyGraphicsPending(
                control: try KittyGraphicsControl.restore(from: pending.control),
                base64Payload: pending.base64Payload
            )
        }
        state.totalImageBytes = snapshot.totalImageBytes
        state.nextImageAccessTick = snapshot.nextImageAccessTick
        return state
    }
}
