...
extension BufferLine {
    func makeCheckpoint() throws -> CheckpointLine {
        let mode: Int
        switch renderMode {
        case .single: mode = 0
        ...
        let encodedCells = try CheckpointCellCodec.encode(data[0..<dataSize])
        return CheckpointLine(
            isWrapped: isWrapped,
            renderMode: mode,
            fillCharacter: try CheckpointCell(fillCharacter),
            cellCount: dataSize,
            cellData: encodedCells.data,
            hyperlinks: encodedCells.hyperlinks,
            images: try (images ?? []).map(CheckpointImage.init),
            generation: generation
        )
    }

    static func restore(from snapshot: CheckpointLine,
                        hyperlinkAtoms: inout [String: TinyAtom]) throws -> BufferLine {
        guard snapshot.cellCount > 0,
              snapshot.cellCount <= TerminalCheckpointLimits.columns else {
            throw TerminalCheckpointError.corrupt("line cell count is invalid")
        }
        let mode: RenderLineMode
        switch snapshot.renderMode {
        case 0: mode = .single
        case 1: mode = .doubleWidth
        case 2: mode = .doubledTop
        case 3: mode = .doubledDown
        default: throw TerminalCheckpointError.corrupt("unknown line render mode")
        }
        let fill = try snapshot.fillCharacter.value()
        let cells = try CheckpointCellCodec.decode(
            data: snapshot.cellData, count: snapshot.cellCount, hyperlinks: snapshot.hyperlinks,
            hyperlinkAtoms: &hyperlinkAtoms
        )
        let line = BufferLine(cols: snapshot.cellCount, fillData: fill, isWrapped: snapshot.isWrapped)
        line.renderMode = mode
        for (index, cell) in cells.enumerated() {
            line.data[index] = cell
        }
        line.images = snapshot.images.map(RestoredTerminalImage.init)
        if line.images?.isEmpty == true {
            line.images = nil
        }
        line.generation = snapshot.generation
        return line
    }
}
